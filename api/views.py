    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if not products.exists():
                return Response({'product_id': 'Данный объект не существует'}, status=status.HTTP_400_BAD_REQUEST)
            obj, is_created = Basket.create_or_update(products.first().id, self.request.user)
            status_code = status.HTTP_201_CREATED if is_created else status.HTTP_200_OK
            serializer = self.get_serializer(obj)
            return Response(serializer.data, status=status_code)
        except KeyError:
            return Response({'product_id': 'Данное поле является обязательным'}, status=status.HTTP_400_BAD_REQUEST)
